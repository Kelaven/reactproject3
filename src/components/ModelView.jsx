...
const ModelView = ({ index, groupRef, gsapType, controlRef, setRotationSize, size, item }) => {
    return (
        <View /* de @react-three/drei */
            index={index}
            id={gsapType}
            className={`w-full h-full ${index === 2} ?? 'right-[-100%]'`} // pour switcher entre le petit téléphone et le grand
        >
            ...
            <group ref={groupRef} name={`${index === 1} ? 'small' : 'large'`} position={[0, 0, 0]}> {/* la position permet de centrer l'iphone */}
